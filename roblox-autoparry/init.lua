...
local function loadModule(path)
	...
	if not fetched then
		error(string.format("[Fleur] could not fetch %s: %s", path, tostring(source)), 0)
	end

	local chunk, syntaxError = loadstring(source, "@" .. path)
	if not chunk then
		error(string.format("[Fleur] syntax error in %s: %s", path, tostring(syntaxError)), 0)
	end

	local built, factory = pcall(chunk)
	if not built then
		error(string.format("[Fleur] error running %s: %s", path, tostring(factory)), 0)
	end

	if type(factory) ~= "function" then
		error(string.format("[Fleur] %s did not return a function(ctx)", path), 0)
	end

	local ran, err = pcall(factory, ctx)
	if not ran then
		error(string.format("[Fleur] error in %s: %s", path, tostring(err)), 0)
	end
end
